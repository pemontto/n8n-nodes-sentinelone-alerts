...
const {
	...
	getUnifiedAlert,
	updateUnifiedAlert,
} = require('../../dist/nodes/SentinelOne/Graphql.js');
...
test('Get sends a fixed scoped document and returns the matching alert', async () => {
	...
	assert.match(captured.body.query, /alert\(id: \$id, scope: \$scope\)/);
...
test('numeric alert and scope IDs are rejected instead of coerced', async (t) => {
	...
	await t.test('numeric scope ID', async () => {
		await assert.rejects(
			getUnifiedAlert(
				context(baseParameters({ scopeIds: [123] }), async () => {
					throw new Error('request should not run');
				}),
				0,
			),
			/every selected.*non-empty ID/i,
		);
	});
});

test('Get rejects a returned alert outside the selected scope without exposing its scope', async () => {
	await assert.rejects(
		getUnifiedAlert(
			context(baseParameters(), async () =>
				envelope('alert', alert({ realTime: { scope: { account: { id: 'other-account' } } } })),
			),
			0,
		),
		(error) => {
			assert.match(error.message, /not found in the selected scope/i);
...
test('Get Many rejects empty guided arrays, unknown filter keys, and reversed dates', async (t) => {
	for (const [name, filters, expected] of [
		['empty array', { severities: [] }, /at least one value/i],
					...
					case 'SentinelOneGetAlert':
						...
						return envelope(
							...
							alert(
								alertReads === 1
									? {}
									: { status: 'RESOLVED', analystVerdict: 'TRUE_POSITIVE_MALWARE' },
							),
						);
